...
class SpotProperties:
    def __init__(self, Position, Width, Height, AverageRadius, Quality):
...
E_Options = []
for x in list(range(70,245,5))+[244]: E_Options.append(x)

Energies = easygui.multchoicebox('Select Energies','Please select the energies that have spot pattern image files', E_Options)
Energies = sorted([int(i) for i in Energies], reverse=True)

dir = easygui.diropenbox('Select Folder Containing Spot Profiles For All Acquired Energies')

if not len(Energies)==len(os.listdir(dir)):
    easygui.msgbox("Number of files in directory does not match number of Energies selected\nPlease re-run the program","File Count Error")

SpotQAImages={} #Creates empty dictionary to put all image data into
HorVals=[]
VertVals=[]
LOG_Output={}
j=0
for foldername in os.listdir(dir):
    ...
    # j+=1
    if os.path.isfile(os.path.join(os.path.join(dir,foldername),'activescript.txt'))==False:
        easygui.msgbox("Folder "+foldername+" Doesn't contain the activescript file required to calculate the image resolution")
        exit()
    if os.path.isfile(os.path.join(os.path.join(dir,foldername),'output.txt'))==False:
        easygui.msgbox("Folder "+foldername+" Doesn't contain the output file required for the spot data")
        exit()
